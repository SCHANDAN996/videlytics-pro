...
def parse_channel_input(input_string):
    ...
    patterns = [
        r'(?:https?:\/\/)?(?:www\.)?youtube\.com\/(?:channel\/|c\/|@|user\/)?([a-zA-Z0-9_-]{2,24})'
    ]
...
def get_channel_details(api_key, channel_input):
    try:
        ...
        if clean_input.startswith('UC') and len(clean_input) == 24:
            ...
            request = youtube.search().list(part="snippet", q=clean_input, type="channel", maxResults=1)
            response = request.execute()
            if not response.get('items'):
                return {'error': f"Channel with handle '{clean_input}' not found."}
            search_param['id'] = response['items'][0]['id']['channelId']
        request = youtube.channels().list(part="snippet,statistics", **search_param)
        response = request.execute()
        if not response.get('items'):
        ...
        return {'error': 'An error occurred with the YouTube API. The API key might be invalid or quota exceeded.'}
